                            ...
                            <!-- start page title -->
                                <div class="col-md-10 mt-4">
                                    ...
                                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                        <h4 class="mb-sm-0 font-size-18">LISTE ARTICLES ({{ $articles->count() }})</h4>
                                    </div>

                                    <div class="row">
                                        <div class="col">
                                            <form action="{{ route('liste.article') }}" method="GET">
                                        ...
                                        <div class="card-body">
                                            <h4 class="card-title mb-4">
                                                <div class="col mb-4 text-right">
                                                    <a href="{{ route('nouvel.article') }}">
                                                        <button type="submit" class="btn btn-primary font-weight-bold">NOUVEL ARTICLE</button>
                                                    </a>
                                                </div>
                                            </h4>
                                            <div class="table-responsive">
                                                <table class="table align-middle table-nowrap mb-0">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            <td><a href="javascript: void(0);" class="text-body fw-bold">{{$article->reference}}</a> </td>
                                                            <td>{{$article->titre}}</td>
                                                            <td>{{$article->date}}</td>
                                                            <td class="d-flex">
                                                                <a href="{{ route('description.article', $article->slug) }}" class="link"><button type="button" class="btn btn-primary btn-sm mr-1"><i class="fa fa-eye text-white"></i></a></button>
                                                                <a href="{{route('article.edit', $article->id)}}" class="link"><button type="submit" class="btn btn-warning btn-sm mr-1"><i class="fa fa-pen text-white"></i></a></button>
                                                                <form onsubmit="return confirmerSuppression({{ $article->id }})" action="{{ route('destroy.article', $article->id) }}" method="POST">
                                                                    @csrf
